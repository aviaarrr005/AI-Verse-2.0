...
SCORE_RECOVERY_RATE = 0.2
SCORE_PENALTY_DISTRACT = 0.2
SCORE_PENALTY_MAJOR = 1.0
...
def generate_frames():
    ...
    global attention_score, current_penalty_reason

    cam = CameraGrabber(src=CAMERA_SOURCE, req_w=1280, req_h=720, buffer_size=2)
    
    current_alert_message = "CALIBRATION: Look straight at camera..."
    current_alert_status = "calib"
    socketio.emit('proctor_alert', {
        'message': current_alert_message,
        'status': current_alert_status
    ...
    try:
        while True:
                    ...
                    if valid and len(face_2d) == 6:
                        ...
                        try:
                            success_pnp, rot_vec, trans_vec = cv2.solvePnP(
                                face_3d, face_2d, cam_matrix, dist_coeffs, flags=cv2.SOLVEPNP_ITERATIVE
                            )
                            if success_pnp:
                                ...
                                if not calibrated:
                                    ...
                                    cv2.putText(frame_small, "Calibrating... Look straight",
                                                (10, FRAME_H - 12), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                                    ...
                                    over_thresh = (
                                        yaw_ema < -YAW_THRESH or
                                        yaw_ema > YAW_THRESH or
                                        pitch_ema > PITCH_DOWN_THRESH or
                                        pitch_ema < PITCH_UP_THRESH
                                    )
                                    ...
                                    if over_thresh:
                                        ...
                                        if (now - distract_start) >= DISTRACT_DWELL_S:
                                            if yaw_ema < -YAW_THRESH:
                                                reason = f"Left {int(abs(yaw_ema))}°"
                                            elif yaw_ema > YAW_THRESH:
                                                reason = f"Right {int(abs(yaw_ema))}°"
                                            elif pitch_ema > PITCH_DOWN_THRESH:
                                                reason = f"Down {int(abs(pitch_ema))}°"
                                            else:
                                                reason = f"Up {int(abs(pitch_ema))}°"
                                    ...
                                    cv2.putText(frame_small, f"Yaw: {int(yaw_ema)}", (10, 28),
                                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                                    cv2.putText(frame_small, f"Pitch: {int(pitch_ema)}", (10, 54),
                                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                            else:
            ...
            if (time.monotonic() - last_yolo_time) * 1000.0 >= YOLO_MIN_MS:
                last_yolo_time = time.monotonic()
                try:
                    with torch.inference_mode():
                        results = yolo_model(
                            frame_small,
                            imgsz=YOLO_IMGSZ,
                            conf=min(PHONE_CONF, OTHER_CONF),
            ...
            if last_device_name and (now - last_device_time) <= DEVICE_HOLD_S:
                if "MULTIPLE" not in frame_alert_message and "AWAY" not in frame_alert_message:
                    if last_device_name != 'keyboard':
                        frame_alert_message = f"ALERT: {last_device_name.upper()} DETECTED!"
                        frame_alert_status = "alert"
                    elif "DISTRACTED" not in frame_alert_message:
                         frame_alert_message = "STATUS: TYPING (Keyboard)"
                         frame_alert_status = "ok"
                         
                if last_device_box:
                    color = (0, 0, 255) if last_device_name != 'keyboard' else (0, 255, 0)
                    cv2.rectangle(frame_small, (last_device_box[0], last_device_box[1]), (last_device_box[2], last_device_box[3]), color, 2)
                    cv2.putText(frame_small, f"{last_device_name}",
                                (last_device_box[0], max(20, last_device_box[1] - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

            if (frame_alert_message != current_alert_message) or (frame_alert_status != current_alert_status):
                if not (not calibrated and "CALIBRATION" in frame_alert_message):
                    current_alert_message = frame_alert_message
                    current_alert_status = frame_alert_status
                    socketio.emit('proctor_alert', {
                        'message': current_alert_message,
